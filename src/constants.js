...
/** Holy Shrimp 스폰 존: 오른쪽 위에서 한 칸 안쪽(왼쪽·아래로 한 칸) */
...
/** Vamp Shrimp 스폰 존: 왼쪽 아래에서 한 칸 안쪽(오른쪽·위로 한 칸) */
...
/** 베이스 코어 초기 수치 — Holy Shrimp가 Vamp Shrimp 스폰 존에 들어올 때마다 1씩 줄고, 0이면 게임오버 */
...
// 위·아래 이동은 Vamp Shrimp 돌진(dash)이 담당한다.
...
/** 진영별 감지 반경(px). Jombie Shrimp는 감지가 둔하다. (향후 성장 요소로 사용 예정) */
...
export const DASH_COOLDOWN_S = 10.0;    // 돌진 종료 후 재사용 대기(초)
...
/** 원거리 공격 사거리(인식 범위) 안에 적이 들어오면 Holy Shrimp는 자세를 잡느라 이속이 1/4로 준다 */
...
export const HUMAN_SPAWN_INTERVAL_S = 0.8; // Holy Shrimp 순차 스폰 간격
